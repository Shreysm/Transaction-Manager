...
/*final edit for initial submission*/
...
public class BTreeFile extends IndexFile implements GlobalConst {
   ...
   {
      PageId pid = null;
      pid = headerPage.get_rootId();
      // Checking if the header page exists or not
      // When the header is empty for the first time
      if(pid.pid < 0){

         //Creating the initial leaf page,which would be root
         ...
         //Set previous pointer and next pointer to null
         newRootPage.setNextPage(new PageId(-1) );
         newRootPage.setPrevPage(new PageId(-1));
         newRootPage.insertRecord(key, rid);      //Inserting the <key,ptr to record = rid> into the leaf page which is indexPage for the first time

         //headerPage.set_rootId(first_root_id);
         ...
         // when the Root Page is already present.
         KeyDataEntry keyEntry = null;

         try {
            keyEntry = _insert(key, rid, headerPage.get_rootId());
         } catch (InvalidSlotNumberException e) {
            e.printStackTrace();
         }
         //In Case Split has occurred
         if(keyEntry!=null){
            ...
            IndexData indexData = (IndexData) keyEntry.data;
            newIndexPage.insertKey(keyEntry.key, indexData.getData());
   ...
   private KeyDataEntry _insert(KeyClass key, RID rid, PageId currentPageId)
      ...
      if(currentPage.getType() == NodeType.INDEX) {
          ...
          KeyDataEntry pushUpEntry = _insert(key, rid, nextPageId);
          
          //If no split has occurred
          if (pushUpEntry == null) {
             return null;
          }
             ...
             if (currentIndexPage.available_space() > BT.getKeyDataLength(pushUpEntry.key, NodeType.INDEX)) {

                IndexData indexData = (IndexData) pushUpEntry.data;
                currentIndexPage.insertKey(pushUpEntry.key, indexData.getData());
                ...
                unpinPage(currentIndexPage.getCurPage(), true);
                 ...
                 newIndexPage.setNextPage(currentIndexPage.getNextPage());
                 currentIndexPage.setNextPage(newIndexPageId);
                 newIndexPage.setPrevPage(currentIndexPage.getCurPage());

                 //To store the current pointer and previous pointer
                 KeyDataEntry tempKeyDataEntry = null, prevTemp = null;
                 RID delRid = new RID();
      
                 //Inserting into new index page and deleting all from old index page
                 for(tempKeyDataEntry = currentIndexPage.getFirst(delRid); tempKeyDataEntry!=null; tempKeyDataEntry = currentIndexPage.getFirst(delRid))
                 {
 					//Inserting into the new index page
 					IndexData indexData = (IndexData)tempKeyDataEntry.data;
 					newIndexPage.insertKey(tempKeyDataEntry.key, indexData.getData());
 					//Deleting from old index page
 					currentIndexPage.deleteSortedRecord(delRid);
 				}
                 
                 //Inserting into old index page to make the split equal
                 for(tempKeyDataEntry = newIndexPage.getFirst(delRid); tempKeyDataEntry!=null; tempKeyDataEntry = newIndexPage.getFirst(delRid))
                 {
 					
                 	if(newIndexPage.available_space()< currentIndexPage.available_space())
                 	{
 					//Inserting half the records back to the old leaf					
 					IndexData indexData = (IndexData)(tempKeyDataEntry.data);	
 					currentIndexPage.insertKey(tempKeyDataEntry.key, indexData.getData());
 					//Removing from the new index page
 					newIndexPage.deleteSortedRecord(delRid);
 					prevTemp = tempKeyDataEntry;
                 	}
 				
 				}
 				
 				tempKeyDataEntry = newIndexPage.getFirst(delRid);
                 
                 
                 //Positive Comparison sends the key to be inserted to new Index Page
                 //else it goes to existing Index Page

                 if (BT.keyCompare(pushUpEntry.key, tempKeyDataEntry.key) > 0)
                 {
                 	IndexData indexData = (IndexData)(pushUpEntry.data);
                 	newIndexPage.insertKey(pushUpEntry.key, indexData.getData());
                 } else
                 {
                 	IndexData indexData = (IndexData)(pushUpEntry.data);
                 	currentIndexPage.insertKey(pushUpEntry.key, indexData.getData());
                 ...
                 newIndexPage.setPrevPage(((IndexData)pushUpEntry.data).getData());
                 //Delete the pushUpEntry from new index page
                 newIndexPage.deleteSortedRecord(delRid);
                 unpinPage(newIndexPageId, true);
                 
                 //Set the pointer info of pushUpEntry to the new Index Page.
 				((IndexData)pushUpEntry.data).setData(newIndexPageId);
 				
 				//Return the pushUpEntry to the higher hierarchy
 				return pushUpEntry;
          ...
          int required_space = BT.getKeyDataLength(key, currentLeafPage.getType());
          int available_space = currentLeafPage.available_space();
          if (available_space >= required_space) {
             ...
             //nothing to send as move up value or index data as split has not occurred
             return null;
          } else {
             // The current leaf page is full.
             // Now split the page into two new leaf pages,
             ...
             int total = currentLeafPage.numberOfRecords();

             int middle = (total/2);
             tempKeyDataEntry = currentLeafPage.getFirst(delRid);

             //Divide the counter into half and copy the second half of records into second Leaf Page
             for (int i=0; i< middle; i++)
             {
                prevTemp = tempKeyDataEntry;
                tempKeyDataEntry = currentLeafPage.getNext(delRid);
             }

             int i = middle;
             while (i < total){

                LeafData leaf_data = (LeafData) tempKeyDataEntry.data;
                //System.out.println(leaf_data.toString());
                newLeafPage.insertRecord(tempKeyDataEntry.key, leaf_data.getData());
                currentLeafPage.deleteSortedRecord(delRid);
                tempKeyDataEntry = currentLeafPage.getCurrent(delRid);
                i= i+1;
             }
             ...
             if (BT.keyCompare(key, prevTemp.key) > 0) {
             ...
             //The first entry of the new split leaf needs to be copied into index
             copyUp = new KeyDataEntry(tempKeyDataEntry.key, newLeafPageId);
   ...
   private boolean NaiveDelete(KeyClass key, RID rid)
      ...
      // Find the leaf page and rid of the given key
      leafPage = findRunStart(key, curRid);
      //If leaf does not contain the required key
      if (leafPage == null)
         return false;

      keyEntry = leafPage.getCurrent(curRid);
      ...
      while (true) {
         ...
         while (keyEntry == null)
         {
            // To traverse to the right of the leaf
            ...
            keyEntry = leafPage.getFirst(firstRID);
         }
         //If the key is greater than the iterator,then break
         ...
         if(leafPage.delEntry(new KeyDataEntry(key, rid)) == false) {
            ...
            //Using recursion to remove all the occurrences of the key.
	   		nextPage = leafPage.getNextPage();
	   		if(nextPage.pid>0)
	   		{
	   		unpinPage(leafPage.getCurPage());
			leafPage=new BTLeafPage(pinPage(nextPage), headerPage.get_keyType());
			keyEntry=leafPage.getFirst(curRid);
			NaiveDelete(key, rid);
	   		}
 	     }
            unpinPage(leafPage.getCurPage());
