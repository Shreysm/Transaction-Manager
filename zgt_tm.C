...
#define TEAM_NO 17    //insert your team number here
...
int zgt_tm::TxRead(long tid, long obno, int thrNum)
 {
  ...
  //Declaring a thread
   ...
   pthread_t thread1;
  //Initialising the node information 
   ...
   int status;
  //Creating a thread and calling the readtx method.

   status = pthread_create(&threadid[thrNum],NULL,readtx,(void*)nodeinfo);
  //If status is not 0, the thread creation is unsuccessful.
...
// write operation is to increment the value by 1. Again the protocol
...
int zgt_tm::TxWrite(long tid, long obno, int thrNum) {
  ...
  int status;
  //Creating a thread and calling the writetx method.
...
int zgt_tm::CommitTx(long tid, int thrNum)
 {
  ...
  int status;
  //Creating a thread and calling the committx method.
...
int zgt_tm::AbortTx(long tid, int thrNum)
 {
  ...
  int status;
  //Create a thread and calling the aborttx method.
